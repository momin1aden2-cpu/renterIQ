...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await model.generateContent([
      ...
      { text: 'Please read every field on this form and pre-fill it with the user data provided above. Output ONLY the field names and values. Do NOT include any branding, app names, watermarks, headers, footers, or generated-by lines. The output must look like a human typed it.' },
    ]);

    let text = result.response.text();
    // Strip any accidental branding references
    text = text.replace(/RenterIQ/gi, '').replace(/renteriq\.com\.au/gi, '').replace(/Generated by.*$/gm, '').trim();
